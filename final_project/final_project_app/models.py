...
class Looks(models.Model):
    ...
    def get_recommendations(cls, user, top_n=5):
        liked_looks = LikedLook.objects.filter(user=user)  # pylint: disable=no-member
        liked_vectors = []
        ...
        if not liked_vectors:
            return cls.objects.exclude(
                id__in=DislikedLook.objects.filter(user=user).values_list('look_id', flat=True)  # pylint: disable=no-member
            ).order_by('?')[:top_n]  # pylint: disable=no-member

        user_pref_vector = np.mean(liked_vectors, axis=0).reshape(1, -1)
        candidates = cls.objects.exclude(
            id__in=liked_looks.values_list('look_id', flat=True)
        ).exclude(
            id__in=DislikedLook.objects.filter(user=user).values_list('look_id', flat=True)  # pylint: disable=no-member
    ...
    def generate_from_preferences(cls, user):
        liked_looks = LikedLook.objects.filter(user=user).select_related('look')  # pylint: disable=no-member
        ...
        top = ClothingItem.objects.filter(category='tops', style=top_style, material=top_material).order_by('?').first()  # pylint: disable=no-member
        bottom = ClothingItem.objects.filter(category='bottoms', style=top_style, material=top_material).order_by('?').first()  # pylint: disable=no-member
        shoes = ClothingItem.objects.filter(category='shoes', style=top_style, material=top_material).order_by('?').first()  # pylint: disable=no-member
    ...
    def get_for_temperature(cls, temp):
        ...
        for (min_t, max_t), range_str in temp_ranges.items():
            if min_t <= temp <= max_t:
                return cls.objects.filter(temp_range=range_str).order_by('?').first()  # pylint: disable=no-member
        return None
